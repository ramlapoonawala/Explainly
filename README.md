...
# ── Parse Gemini structured feedback response ─────────────────────────────
...
# ── Analyse student response with Gemini Vision ───────────────────────────
...
Analyse this question a teacher has set for students:
"{question}"
...
- Transcript: {r.transcript[:200] if r.transcript else 'Written only'}
...
If none write: No common notation errors detected]
...
their thinking across the class]

SUBMISSION_PATTERN: [One sentence about submission modes used]
...
SUGGESTED_NEXT_MOVE: [Two to three specific sentences naming the
...
async def create_session(
    ...
    if not question and not (question_image and question_image.filename):
        raise HTTPException(
            status_code=400,
            detail="Either a question text or question image must be provided"
        )
    ...
    if question_image and question_image.filename:
        ...
        path = os.path.join(UPLOAD_DIR, question_image_filename)
        ...
        if ext == "pdf":
            ...
            pdf_to_image(path, png_path)
            question_image_filename = png_filename
            path = png_path

        # Extract question from image if no text provided
        if not question:
            question = extract_question_from_image(path)

    # Generate AI question analysis
    question_analysis = analyse_question(question)
...
async def submit_response(
    ...
    # Uploaded photo takes priority over canvas
